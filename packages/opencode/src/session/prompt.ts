...
NOTE: At any point in time through this workflow you should feel free to ask the user questions or clarifications. Don't make large assumptions about user intent. The goal is to present a well researched plan to the user, and tie any loose ends before implementation begins.
      ...
      const resolveTools = Effect.fn("SessionPrompt.resolveTools")(function* (input: {
        ...
        const context = (args: any, options: ToolExecutionOptions, toolName?: string): Tool.Context => ({
          sessionID: input.session.id,
      ...
      const prompt: (input: PromptInput) => Effect.Effect<MessageV2.WithParts> = Effect.fn("SessionPrompt.prompt")(
        function* (input: PromptInput) {
          // Reset classifier denial tracking on each new user prompt
          // to prevent accumulated denials from permanently disabling the classifier.
          Classifier.reset()
          ...
          while (true) {
            ...
            const outcome: "break" | "continue" = yield* Effect.onExit(
              Effect.gen(function* () {
                ...
                const isSubtask = !!session.parentID
                const [skills, env, instructions, modelMsgs, permanent, recent] = yield* Effect.promise(() =>
                  Promise.all([
                    SystemPrompt.skills(agent),
                    SystemPrompt.environment(model),
                    InstructionPrompt.system(),
                    MessageV2.toModelMessages(msgs, model),
                    // only load memory content for root sessions, not subtasks
                    !isSubtask && Memory.enabled() ? Memory.readPermanent() : Promise.resolve(""),
                    !isSubtask && Memory.enabled() ? Memory.readRecent() : Promise.resolve(""),
                  ]),
                )
                const memory: string[] = []
                if (Memory.enabled()) {
                  const mpath = Memory.permanentPath()
                  memory.push(SystemPrompt.memory(mpath))
                  // Always inject MEMORY.md path so the AI can edit it;
                  // include content when available.
                  if (mpath) {
                    const block = permanent
                      ? [
                          "<permanent-memory>",
                          `MEMORY.md path: ${mpath}`,
                          "Use the Edit or Write tool on this path to update permanent memory.",
                          "",
                          permanent,
                          "</permanent-memory>",
                        ]
                      : [
                          "<permanent-memory>",
                          `MEMORY.md path: ${mpath}`,
                          "The file is currently empty. Write user info, preferences, and key decisions here using the Edit or Write tool.",
                          "</permanent-memory>",
                        ]
                    memory.push(block.join("\n"))
                  }
                  if (recent) {
                    memory.push(
                      [
                        "<recent-memory>",
                        "The following is content from recent daily memory files (memory/YYYY-MM-DD.md).",
                        "These capture session summaries and key context from recent days.",
                        "",
                        recent,
                        "</recent-memory>",
                      ].join("\n"),
