...
export namespace Config {
  ...
  export const Info = z
    .object({
      ...
      memory: z
        .object({
          enabled: z.boolean().optional().describe("Enable cross-session memory (default: false)"),
          provider: z.string().optional().describe("Embedding provider to use (openai, gemini, voyage, mistral, ollama)"),
          model: z.string().optional().describe("Embedding model name override"),
        })
        .optional()
        .describe("Cross-session memory configuration"),
      classifier: z
        .object({
          enabled: z.boolean().optional().describe("Enable AI permission classifier for auto-approving safe tool calls (default: false)"),
          timeout: z.number().int().positive().optional().describe("Timeout in milliseconds for each classifier stage (default: 5000)"),
          model: z.string().optional().describe("Model to use for classification (e.g. 'anthropic/claude-haiku'). Falls back to provider's small model."),
          allowlist_override: z.array(z.string()).optional().describe("Additional tool names to always auto-approve"),
          blocklist_override: z.array(z.string()).optional().describe("Tool names to never auto-approve (always ask user)"),
          denial_threshold: z
            .object({
              consecutive: z.number().int().positive().optional().describe("Consecutive denials before disabling classifier (default: 3)"),
              total: z.number().int().positive().optional().describe("Total denials before disabling classifier (default: 20)"),
            })
            .optional()
            .describe("Thresholds for automatic classifier disabling after repeated denials"),
        })
        .optional()
        .describe("AI permission classifier configuration for auto-approving safe tool calls"),
      experimental: z
        .object({
